...
class User_Manager {
    ...
    public static create_Email_Password_Account(email: string, password: string, account_custom_attributes?: unknown): Promise<{ account_ID: string }> {

        return new Promise((resolve, reject) => {

            HTTP_Fetcher.Web_Presence.User.create_Email_Password_Account(email, password, account_custom_attributes).then((fetch_response) => {
    ...
    public static Email_Password_Login(email: string, password: string, keep_session_alive: boolean): Promise<{ token: string, refresh_token: string }> {
